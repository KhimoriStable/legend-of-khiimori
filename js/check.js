...
function validateHorses(horses) {
    ...
    horses.forEach((horse, index) => {
        ...
        if (!Array.isArray(horse.stats)) {
            ...
            const notMaxedStats = horse.stats.filter(stat => {
                return Number(stat.current) < Number(stat.max);
            });

            const allStatsMaxed = horse.stats.length > 0 && notMaxedStats.length === 0;

            if (horse.status === "ready" && !allStatsMaxed) {
                warnings.push(
                    `${horse.id || horse.name}: статус “Готова к разведению”, но не все характеристики на максимуме.`
                );
            }

            if (horse.status !== "ready" && allStatsMaxed) {
                warnings.push(
                    `${horse.id || horse.name}: все характеристики на максимуме, но статус ещё “Не готова”.`
                );
            }
        }
    });
